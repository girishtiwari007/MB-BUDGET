(function () {
  ...
  function monthValue(names, year, index){ if(year === years().at(-1) && index >= completedCount()) return null; return names.reduce((total,name) => total + Number(source()[name]?.[year]?.[index] || 0),0); }
  ...
  function chart(names, chosenYears){
    ...
    const series=chosenYears.map((year,index) => ({year,color:["#1f4e79","#0a8a82","#a66321","#8b3f65"][index%4], values:months.map((_,i) => monthValue(names,year,i))}));
