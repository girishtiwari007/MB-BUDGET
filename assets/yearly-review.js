(function () {
  ...
  function periodIndexes(){ return state.period === "month" ? [months.indexOf(state.month)] : state.period === "quarter" ? (quarters[state.quarter] || quarters.Q1) : months.map((_,i) => i); }
  ...
  function value(values, year){ let indexes = periodIndexes(); if(year === years().at(-1)) indexes = indexes.filter((i) => i < completedCount()); return indexes.reduce((total,i) => total + Number(values?.[year]?.[i] || 0),0); }
  function monthValue(names, year, index, dataSource=source()){ if(year === years().at(-1) && index >= completedCount()) return null; return names.reduce((total,name) => total + Number(dataSource[name]?.[year]?.[index] || 0),0); }
  function checkboxList(host, all, set, type, allText){ host.innerHTML = `<label class="check-all"><input type="checkbox" data-type="${type}" data-all="true" ${set.size === all.length ? "checked" : ""}> ${esc(allText)}</label>${all.map((v) => `<label><input type="checkbox" data-type="${type}" value="${esc(v)}" ${set.has(v) ? "checked" : ""}> ${esc(v)}</label>`).join("")}`; }
  function sync(){
  ...
  function chart(names, chosenYears, dataSource=source()){
    ...
    return `<svg class="trend-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Monthly completed actual trend">${lines}${months.map((m,i) => `<text x="${x(i)}" y="${height-13}" text-anchor="middle" class="axis">${m}</text>`).join("")}${paths}</svg><div class="legend">${series.map((s) => `<span><i style="background:${s.color}"></i>${esc(s.year)}</span>`).join("")}</div>`;
  }
  ...
  function attentionClass(row,total){
    const share=total ? Math.abs(row.total)/Math.abs(total)*100 : 0;
    const values=row.values.map((value)=>Number(value||0));
    const latest=values.at(-1)||0, previous=values.length>1 ? values.at(-2)||0 : 0;
    const jump=previous ? (latest-previous)/Math.abs(previous)*100 : 0;
    if(row.total<0 || share>=25 || jump>=25) return "attn-red";
    if(share>=12 || jump>=10) return "attn-amber";
    return "attn-green";
  }
